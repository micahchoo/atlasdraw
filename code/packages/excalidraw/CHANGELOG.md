...
We've transitioned from `UMD` to `ESM` bundle format. Our new `dist` folder inside `@atlasdraw/excalidraw` package now contains only bundled source files, making any dependencies tree-shakable. The package comes with the following structure:
...
import * as excalidrawLib from "@atlasdraw/excalidraw";
...
<link
  rel="stylesheet"
  href="https://esm.sh/@atlasdraw/excalidraw@0.18.0/dist/dev/index.css"
/>
...
<script type="module">
  ...
  import * as ExcalidrawLib from "https://esm.sh/@atlasdraw/excalidraw@0.18.0/dist/dev/index.js?external=react,react-dom";
</script>
```
...
All fonts are automatically loaded from the [esm.run](https://esm.run/) CDN. For self-hosting purposes, you'll have to copy the content of the folder `node_modules/@atlasdraw/excalidraw/dist/prod/fonts` to the path where your assets should be served from (i.e. `public/` directory in your project). In that case, you should also set `window.EXCALIDRAW_ASSET_PATH` to the very same path, i.e. `/` in case it's in the root:
...
// after
import { CaptureUpdateAction } from "@atlasdraw/excalidraw";
updateScene({
  elements,
  appState,
...
- Prefer user defined coordinates and dimensions when creating a frame using [`convertToExcalidrawElements`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/excalidraw-element-skeleton#converttoexcalidrawelements) [#8517](https://github.com/excalidraw/excalidraw/pull/8517)
...
- Export types for @atlasdraw/utils [#7736](https://github.com/excalidraw/excalidraw/pull/7736)
...
- Export [`getCommonBounds`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/utils#getcommonbounds) helper from the package [#7247](https://github.com/excalidraw/excalidraw/pull/7247).
...
- The `Ref` support has been removed in v0.17.0 so if you are using refs, please update the integration to use the [`excalidrawAPI`](http://localhost:3003/docs/@atlasdraw/excalidraw/api/props/excalidraw-api) [#7251](https://github.com/excalidraw/excalidraw/pull/7251).
...
- [`useDevice`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/utils#usedevice) hook's return value was changed to differentiate between `editor` and `viewport` breakpoints. [#7243](https://github.com/excalidraw/excalidraw/pull/7243)
...
- Added [`props.validateEmbeddable`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/props#validateembeddable) to customize embeddable src url validation. [#6691](https://github.com/excalidraw/excalidraw/pull/6691)
- Add support for `opts.fitToViewport` and `opts.viewportZoomFactor` in the [`ExcalidrawAPI.scrollToContent`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/props/excalidraw-api#scrolltocontent) API. [#6581](https://github.com/excalidraw/excalidraw/pull/6581).
...
- Use subdirectory for @atlasdraw/excalidraw size limit [#6787](https://github.com/excalidraw/excalidraw/pull/6787)
...
- [`ExcalidrawAPI.scrollToContent`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/props/excalidraw-api#scrolltocontent) has new opts object allowing you to fit viewport to content, and animate the scrolling. [#6319](https://github.com/excalidraw/excalidraw/pull/6319)
...
- [`restoreElements`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/utils/restore#restoreelements) API now takes an optional parameter `opts` which currently supports the below attributes

```js
{ refreshDimensions?: boolean, repairBindings?: boolean }
```

The same `opts` param has been added to [`restore`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/utils/restore#restore) API as well.

For more details refer to the [docs](https://docs.excalidraw.com)

#### BREAKING CHANGE

- The optional parameter `refreshDimensions` in [`restoreElements`](https://docs.excalidraw.com/docs/@atlasdraw/excalidraw/api/utils/restore#restoreelements) has been removed and can be enabled via `opts`
...
- Added exportPadding to PNG (blob) export in @atlasdraw/utils [#5626](https://github.com/excalidraw/excalidraw/pull/5626)
...
- Export types for the package so now it can be used with typescript [#3337](https://github.com/excalidraw/excalidraw/pull/3337). The types are available at `@atlasdraw/excalidraw/types`.
...
- Expose `window.EXCALIDRAW_ASSET_PATH` which host can use to load assets from a different URL. By default it will be loaded from `https://unpkg.com/@atlasdraw/excalidraw{currentVersion}/dist/`[#3068](https://github.com/excalidraw/excalidraw/pull/3068).
...
First release of `@atlasdraw/excalidraw`
