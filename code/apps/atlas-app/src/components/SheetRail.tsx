...
export function SheetRail({ excalidrawAPI }: SheetRailProps) {
  ...
  // within the rail. Anchored to the open tab so Tab lands somewhere meaningful.
  const [focusName, setFocusName] = useState<string | null>(null);
  const buttonsRef = useRef(new Map<string, HTMLButtonElement>());

  const activeIndex = tabs.findIndex((tab) => tab.name === openTab);
  const focusIndex = (() => {
    const byFocus = tabs.findIndex((tab) => tab.name === focusName);
    if (byFocus >= 0) {
      return byFocus;
    }
  ...
  const moveFocus = useCallback(
    (index: number) => {
      ...
      setFocusName(next.name);
      buttonsRef.current.get(next.name)?.focus();
    },
    [tabs],
  );
  ...
  if (!excalidrawAPI || tabs.length === 0) {
    return null;
  }
