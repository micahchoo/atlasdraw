...
| T19 — Bench harness + Phase 1 baseline | SHIPPED | Minimal harness shipped 2026-05-05 — `code/bench/` workspace `@atlasdraw/bench` with synthetic GeoJSON fixtures (mulberry32-seeded), 3-warmup + 20-iteration timing, three scenarios (parse 1k / parse 10k / parse + requireHomogeneousGeometry 10k). Baseline written to `code/bench/results/phase-1-baseline.json`. Maintainer decision: ship minimal — perf-investigation 10-phase rigor deferred. Closes `atlasdraw-f1fa`. |
| T20 — Phase 2 acceptance gate run | UNBLOCKED, OUTSTANDING | T19 baseline now exists; T20's actual scope is larger — Phase 2 scenario (50k features + 5k annotations), `phase-2-with-data-layers.json`, `ci-gate.ts` assertion, `.github/workflows/ci.yml` step. `atlasdraw-1315` open. |
...
| T31 — LayerPanel CSS module retrofit (post-plan) | SHIPPED | `b8bb015` (feat(sidebar): mount LayerPanel via vendored registerSidebarTab API). Bundled together: (a) the 2nd vendored fork queued under `atlasdraw-90a5` landed today (`registerSidebarTab` on `ExcalidrawImperativeAPI`) — `atlasdraw-90a5` resolved affirmatively rather than deferred to Phase 4; (b) LayerPanel rewritten body-only into `LayerPanel.module.css` with inline SVG icons (currentColor), `data-testid`s on every interactive control, and the atlas-active-blue iconButton styling; (c) `atlasdraw-ui-conventions` skill updated — File Placement table swaps stale `renderSidebar` for `registerSidebarTab`; color table extended with row-separator, secondary-metadata, and the data/annotation kind-badge tokens. Verified: atlas-app vitest 91/91, monorepo tsc PASS. |
...
| T28 | architectural orphans | T28.2 (`atlasdraw-cc43`) decided 2026-05-05: stays as-is since T24 chose reject. T28.x (`atlasdraw-d592` anti-pattern detector scope) PARTIAL: project-local intent declared at `.claude/anti-pattern-scope.txt` (5 atlas / 325 vendored / 26 other in latest scan); global detector tooling change needed to honor it. |
