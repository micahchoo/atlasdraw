...
| 11 | Reopen a corrupt `.atlasdraw` | Error message telling me the file is bad | **Nothing.** Picker closes, no dialog, no toast — only `console.warn("[atlasdraw] openFromDisk failed", AtlasdrawFormatError…)`. User assumes the app ignored their click | 3 | pending | re-walked: corrupt open → error toast; good open → success toast |
